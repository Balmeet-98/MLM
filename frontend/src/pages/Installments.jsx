...
export default function Installments() {
      ...
      openRazorpayCheckout({
        ...
        onSuccess: async (paymentResponse) => {
          try {
            ...
            const paidInst = data.installments.find((i) => i.month_number === monthNumber);
            setPaymentSuccess({
              monthNumber,
              amount: paidInst?.amount || 1200,
            });
            fetchData();
          } catch (err) {
            toast.error(
  ...
  const dismissSuccess = () => setPaymentSuccess(null);

  return (
    <div className="space-y-6">
      {paymentSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="card max-w-md w-full text-center p-8 shadow-2xl">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Month {paymentSuccess.monthNumber} installment of ₹{Number(paymentSuccess.amount).toLocaleString('en-IN')} received.
